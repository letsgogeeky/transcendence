...
import { updateScore, findPolygonSide, paddles } from "./stage.ts";
...
export class Ball {
  ...
  step() {
	...
	this.speed += 0.01;
	if (this.position.length() > global.radius) {
		if (paddles.length == 2 && this.position.length() < global.radius * 1.7)
			return;
		this.reset(true);
	}

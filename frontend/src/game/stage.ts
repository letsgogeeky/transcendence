...
function getRegularPolygonPoints(n: number, sideLength: number): BABYLON.Vector3[] {
  ...
  window.radius = sideLength / (2 * Math.sin(Math.PI / n));

  for (let i = 0; i < n; i++) {
    const x = window.radius * Math.sin(i * angleStep);
    const y = window.radius * Math.cos(i * angleStep);
...
export const createStage = function(scene: BABYLON.Scene, canvas: HTMLCanvasElement): void {
